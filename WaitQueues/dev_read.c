...
ssize_t dev_read(struct file *filep, char __user *buff, size_t size, loff_t * loff)
{
	...
	struct Dev *ldev;
	int noctr;
	int noctr1, nocnr, bytes, j, q2s, b2s;
	...
	wait_event_interruptible(ldev->waitQ, atomic_read(&ldev->datasize) > 0);
	...
	noctr = atomic_read(&ldev->datasize);// - *loff;
	...
	up(&ldev->sem);
        atomic_sub(bytes, &ldev->datasize);
	printk(KERN_INFO "END: %s datasize = %d\n", __func__, atomic_read(&ldev->datasize));
